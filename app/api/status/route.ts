...
export async function GET() {
  ...
  let isHistoricalIngesting = false;
  let lastUpdated = null;
  let apiStatus = "online";

  try {
    // Read flags and api status from DB
    const [statusRes, histStatusRes, updateRes, apiStatusRes] = await Promise.all([
      pool.query("SELECT value FROM system_config WHERE key = 'is_ingesting'"),
      pool.query("SELECT value FROM system_config WHERE key = 'is_historical_ingesting'"),
    ...
    isHistoricalIngesting = histStatusRes.rows[0]?.value === 'true';
